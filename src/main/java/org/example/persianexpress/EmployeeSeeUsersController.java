...
public class EmployeeSeeUsersController {
    ...
    public void initialize() throws SQLException {
        ...
        statement = connection.prepareStatement("select *from CustomersInfo");
        ResultSet resultSet = statement.executeQuery();
        while (resultSet.next()){
            User user = new User(resultSet.getInt("CustomerID"));
            ...
            user.setPhNumber(resultSet.getNString("PhoneNumber"));
            user.sethPhNumber(resultSet.getNString("HomePhoneNumber"));
    ...
    private void ShowUsers(ArrayList<User> users){
        for (User user:users) {
            ...
            Label uFatherNamelbl = new Label("نام پدر : ");
            Label uFatherNametxt = new Label(user.getFatherName());
            ...
            uFatherNamelbl.getStyleClass().add("labels2");
            uFatherNametxt.getStyleClass().add("labels2");
            ...
            vBox3.getChildren().addAll(userNamelbl , uFatherNamelbl , userBPlacelbl , userHPhNumlbl , userPCodelbl);
            vBox4.getChildren().addAll(userNametxt , uFatherNametxt , userBPlacetxt , userHPhNumtxt , userPCodetxt);
            ...
            disableAcc.setOnAction(new EventHandler<ActionEvent>() {
                ...
                public void handle(ActionEvent event) {
                    try {
                        statement = connection.prepareStatement("select Access from CustomersInfo where CustomerID = ?");
                        ...
                        if (access){
                            ...
                            statement = connection.prepareStatement("update CustomersInfo set Access = ? where CustomerID = ?");
    ...
    @FXML
    private void onAddUserClicked(ActionEvent event) throws IOException {
